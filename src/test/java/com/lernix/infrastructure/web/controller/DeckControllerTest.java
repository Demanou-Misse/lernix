...
import com.lernix.application.usecase.deck.*; // Vérifie bien cet import vers ton nouveau package
...
class DeckControllerTest {
    ...
    // SENIOR: You MUST mock ALL use cases declared in the Controller constructor
    ...
    @Test
    @DisplayName("POST /api/v1/decks - Should return 201 when creation is successful")
    void shouldCreateDeckViaApi() throws Exception {
        UUID userId = UUID.randomUUID();
        CreateDeckRequest request = new CreateDeckRequest(userId.toString(), "English Vocabulary", "Description");
        Deck mockDeck = Deck.create(new UserId(userId), new DeckTitle("English Vocabulary"), "Description");
        ...
        mockMvc.perform(post("/api/v1/decks")
                ...
                .andExpect(jsonPath("$.data.title").value("English Vocabulary"));
    }
}
